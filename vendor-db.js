...
class VendorDatabase {
  ...
  async enqueueFailedSync(vendorId, payload) {
    try {
      ...
      queue.push({ id: vendorId, payload, ts: Date.now() });
      localStorage.setItem("devka_sync_queue", JSON.stringify(queue));
    } catch (e) {
    ...
    try {
      ...
      for (const item of queue) {
        try {
          ...
          console.warn(`DevkaCloud: Retry sync failed for ${item.id}:`, e.message || e);
          remaining.push(item);
        }
      }
      localStorage.setItem("devka_sync_queue", JSON.stringify(remaining));
    } catch (e) {
      console.warn("DevkaCloud: processSyncQueue error:", e);
    }
  }

  async cacheVendorLocally(vendor) {
    if (!vendor || !vendor.id) return;
    if (this.cloudOnly) return; // skip local cache when cloud-only mode is enabled
  ...
  async getAllVendors() {
    // 1. Check if Firestore is available and fetch remote records (with 2000ms timeout)
    const db = window.DevkaCloud?.getFirestore();
    if (db) {
      try {
        ...
        console.warn("Firestore getAll query notice (using local fast-cache):", err.message);
      }
    }
  ...
  async saveVendor(vendor) {
    ...
    // 1. Save in local cache first for immediate and 100% reliable local registration
    if (!this.cloudOnly) await this.cacheVendorLocally(vendor);

    // 2. Persist to Firebase Firestore asynchronously without blocking submission
    const db = window.DevkaCloud?.getFirestore();
    if (db) {
      try {
        ...
        if (payload.documents) {
          for (const key in payload.documents) {
            ...
            if (d && d.data && d.data.length > 200000) {
              delete d.data; // Avoid Firestore 1MB document quota limit
            }
          }
        }
        db.collection("vendors").doc(vendor.id).set(payload, { merge: true })
          .then(() => {
            console.log(`DevkaCloud: Vendor ${vendor.id} synchronized with Firestore.`);
            // Try processing any queued items after a successful write
            try { this.processSyncQueue(); } catch (e) {}
          })
          .catch((err) => {
            console.warn("Firestore sync notice (stored to local queue):", err && err.message ? err.message : err);
            try { this.enqueueFailedSync(vendor.id, payload); } catch (e) {}
          });
      } catch (e) {
        console.warn("Firestore payload prep notice:", e);
      }
    }
  ...
  /**
   * Purge legacy demo seed vendors (Smit Enterprise, Rahul Sharma)
   */
