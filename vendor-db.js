...
class VendorDatabase {
  ...
  async saveVendor(vendor) {
    ...
    await this.cacheVendorLocally(vendor);
    ...
    if (db) {
      try {
        ...
        db.collection("vendors").doc(vendor.id).set(payload, { merge: true })
          .then(() => console.log(`DevkaCloud: Vendor ${vendor.id} synchronized with Firestore.`))
          .catch((err) => console.warn("Firestore sync notice (record stored locally):", err.message));
      } catch (e) {
